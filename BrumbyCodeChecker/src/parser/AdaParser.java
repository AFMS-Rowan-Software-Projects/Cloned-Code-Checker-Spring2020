 ...
 */
	 ...
	 * 
	 * TODO make it work for ada. Change identifiers
	 * 
	 * @param file
	 * @param start
	 * @return indices
	 */
	public static int[] findAdaMethod(ArrayList<Token> file, int start) {
		// Returns start and end index (inclusively) of a method within a file
		int[] indices = new int[2];
		int lbracecounter;
		// Smallest possible method is 7 tokens long
		for (indices[0] = start; indices[0] < (file.size() - 7); indices[0]++) {
			...
			// Match starting pattern of method - [procedure Identifier is]
			if (((file.get(indices[0]) instanceof TTokens))
					&& ( (file.get(indices[0] + 1) instanceof TTokens) || (file.get(indices[0] + 1) instanceof Token))
					&& (file.get(indices[0] + 2) instanceof Token)) {
				// Capture everything between ( and ) of a procedure
				while (!(file.get(indices[1]) instanceof Token) && (indices[1] < (file.size() - 3))) {
					indices[1]++;
				}
				indices[1]++;
				...
				if (!(file.get(indices[1]) instanceof TTokens)) { // { should be immediately after )
					continue;
				}
				else {
					...
					for (indices[1]++; (lbracecounter != 0) && (indices[1] < file.size()); indices[1]++) {
						...
						if (file.get(indices[1]) instanceof TTokens) {
							lbracecounter++;
						}
						else if (file.get(indices[1]) instanceof TTokens) {
							lbracecounter--;
						}
					}
					if (lbracecounter == 0) {
	...
	public static ArrayList<Token> sanitize(ArrayList<Token> t_large) {
		...
		for (int i = 0; i < tlist.size(); i++) {
			if ((tlist.get(i) instanceof InvalidToken) || (tlist.get(i) instanceof TBlank)
					|| (tlist.get(i) instanceof TComment) || (tlist.get(i) instanceof TTokens) || (tlist.get(i) instanceof Token)) {
				tlist.remove(i);
