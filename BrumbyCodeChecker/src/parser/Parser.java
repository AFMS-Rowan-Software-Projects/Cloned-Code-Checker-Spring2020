 ...
 */
	...
	public static int[] findAdaMethod(ArrayList<Token> file, int start) {
		...
		for (indices[0] = start; indices[0] < (file.size() - 7); indices[0]++) {
			...
			if (((file.get(indices[0]) instanceof TKeyword) || (file.get(indices[0]) instanceof TIdentifier))
					&& ( (file.get(indices[0] + 1) instanceof TUnknown) || (file.get(indices[0] + 1) instanceof TIdentifier))
					&& (file.get(indices[0] + 2) instanceof TKeyword)) {
				// Capture everything between ( and ) of a procedure
				while (!(file.get(indices[1]) instanceof TKeyword) && (indices[1] < (file.size() - 3))) {
					indices[1]++;
				}
				indices[1]++;
				// EOF reached (End of File)
				...
				if (!(file.get(indices[1]) instanceof TKeyword)) { // { should be immediately after )
					continue;
				}
				else {
					// Match up braces with one another and only terminate when finding the method's '}'
					...
					for (indices[1]++; (lbracecounter != 0) && (indices[1] < file.size()); indices[1]++) {
						...
						if (file.get(indices[1]) instanceof TKeyword) {
							lbracecounter++;
						}
						else if (file.get(indices[1]) instanceof TKeyword) {
							lbracecounter--;
						}
					}
					if (lbracecounter == 0) {
